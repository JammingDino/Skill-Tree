...
public class SkillConditionListAdapter implements JsonDeserializer<List<SkillCondition>>, JsonSerializer<List<SkillCondition>> {
    ...
    public JsonElement serialize(List<SkillCondition> src, Type typeOfSrc, JsonSerializationContext context) {
        ...
        for (SkillCondition condition : src) {
            jsonArray.add(serializeCondition(condition, context));
        }
        return jsonArray;
    }

    // --- SHARED HELPER METHOD ---
    public static JsonObject serializeCondition(SkillCondition condition, JsonSerializationContext context) {
        JsonObject obj = new JsonObject();

        // --- LOGIC ---
        if (condition instanceof AndCondition c) {
            obj.addProperty("type", "jd_skill_tree:and");
            // For nested lists, we can still use the context because GSON knows List<SkillCondition> maps to this adapter
            obj.add("conditions", context.serialize(c.getConditions()));
        }
        else if (condition instanceof OrCondition c) {
            obj.addProperty("type", "jd_skill_tree:or");
            obj.add("conditions", context.serialize(c.getConditions()));
        }
        else if (condition instanceof NotCondition c) {
            obj.addProperty("type", "jd_skill_tree:not");
            obj.add("condition", serializeCondition(c.getCondition(), context));
        }
